...
handlers = [logging.FileHandler('log.out', mode='w+'), logging.StreamHandler()]
...
def calc_util(bm, no_parallel_benchmarks, r_range, c_range):
    ...
    N_par = N // no_parallel_benchmarks
    ...
    for layer_name in layer_dims:
        ...
        total_time += np.multiply(np.ceil( np.divide(folds, N_par) ), w_stall)

    T_peak = calculate_peak_throughput(r_range,c_range,N,freq)
    ops_per_cycle = no_parallel_benchmarks * bm.no_ops / total_time
    ops_per_sec = ops_per_cycle * freq

    util = ops_per_sec / 1e12 / T_peak 

    return util

def calc_atpp(bm, no_parallel_benchmarks, r_range, c_range):
    layer_dims = bm.layer_dims
    no_layers = len(layer_dims)
    assert no_layers>0, "Empty layer list"
    ...
    atpp = no_parallel_benchmarks * bm.no_ops / total_time / p_peak #ops/cycle/W
    return atpp
...
def plot_design_space(atpps):
    ...
    for ind, ds in enumerate([ds_bert, ds_cnn, ds_mix]):
        ...
        r_peak = r_range[i]
        c_peak = c_range[j]
        logging.info("Peak at {}x{}".format(r_peak, c_peak))
        peaks[names[ind]] = (r_peak, c_peak)
        ...
        for r,c in baselines+[(r_peak,c_peak)]+[(16,16), (32,32), (64,64), ]:
            ...
            T[(r,c)] = ds[i,j]
            logging.info("{}x{}: {}".format(r,c,T[(r,c)]))
...
# @profile
def run_design_space(batch_size, no_parallel_benchmarks):    
    ...
    for model_name in bert_models:
        ...
        for no_seq in seq_list:
            ...
            atpp_np = calc_atpp(bm, no_parallel_benchmarks, r_range, c_range)
            del bm 

            atpps[model_name][no_seq] = atpp_np.tolist()

            elapsed = time.time() - start
            logging.info("Model: {} no_seq: {} \t elapsed: {}".format(model_name, no_seq ,elapsed))
    ...
    for model_name in cnn_models:
        ...
        for imsize in imsize_list:
            ...
            elapsed = time.time() - start
            logging.info("Model: {} \t elapsed: {}".format(model_name ,elapsed))
...
def main():
    peaks = {}
    for no_parallel_benchmarks in [1]:
        for batch_size in [1,2,4,8,16]:    
            atpps = run_design_space(batch_size, no_parallel_benchmarks)
            p = plot_design_space(atpps)
            peaks[(batch_size, no_parallel_benchmarks)] = p["Mix"]
    logging.info(peaks)
